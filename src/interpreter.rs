...
fn parse_lmc_asm<'a>(
...
) -> InterpreterResult<(Vec<RawInst>, SymbolTable)> {
    ...
    let insts = tokens
        ...
        .map(|(i, line)| {
            ...
            if let Some(label) = label {
                if let Some(other) = symbol_table.get(label.source) {
                    return Err(InterpreterError::DuplicateLabel(
                        label.source,
                        other.token.span,
                        label.span,
                    ));
        ...
        .collect::<InterpreterResult<Vec<RawInst>>>()?;
                ...
                .map(|(span, operand)| {
                    operand.try_map_value(|v| match v {
                        ...
                        OperandValue::Label(label) => symbol_table
                            ...
                            .ok_or_else(|| InterpreterError::UndefinedLabel(&label, span)),
                    })
                })
                .transpose()?;
...
pub fn compile_lmc_asm<'a>(asm: &'a str) -> InterpreterResult<()> {
    ...
    let insts = process_labels(raw_insts, symbol_table)?;
